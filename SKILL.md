...
每則新聞自動補上 **sentiment(利多/利空/中性)+ confidence + events(20 類:法說/
購併/警示/獲利預警/產能/訂單/訴訟/股利/增資/減資/公開收購/漲停/跌停/踩雷/
財報/營收/庫藏股/法人動向/財務危機/重大訊息)**,規則層、純標準庫、可由
`tag_sentiment=False` 關掉。
...
**confidence 是「字面一面倒的程度」,不等於「判斷正確率」** —— `< 0.3` 固然要自己
看內文,**高 confidence(0.6~0.8)也可能是高信心的反向誤判**(尤其「程度縮小修飾負面詞」
「澄清/否認」「漲多拉回/見光死」等情境,已部分處理但難窮盡);規則層仍不懂完整語境、
不會反諷,「競爭對手搶單」這類主體判斷只部分涵蓋。**最終一律以內文為準**。
...
> 表中「N case」指測試**內部子案例/斷言數**(非 `def test_` 函式數),與各測試自印的 pass 數一致。
